 ...
 * 解決を PostgreSQL worker schema で検証するテスト。
 */
...
describe('DocumentShareRepository', () => {
  let db: SqlDatabase;
  ...
  it('resolves user, group, and role matches with case-insensitive group/role', async () => {
    ...
    expect(await repo.resolvePermission('notebook', 'nb_1', accessor('dave', [], 'analyst'))).toBe(
      'view',
    );
  ...
  it('prefers edit over view when multiple shares match', async () => {
    ...
    expect(await repo.resolvePermission('saved_query', 'sq_2', accessor('bob', [], 'member'))).toBe(
      'edit',
    );
  });
  ...
  it('lists accessible document ids for an accessor', async () => {
    ...
    const saved = await repo.listAccessibleDocumentIds('saved_query', accessor('bob', ['TEAM-A']));
