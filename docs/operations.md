...
| `RESULT_PROFILE_DUCKDB_ENABLED`   | `false`                           | Parquet を DuckDB で直接 profile する経路。明示的に `true` にした場合だけ有効。negative capability cache の TTL は `RESULT_PROFILE_DUCKDB_NEGATIVE_CACHE_TTL_MS`（既定 `60000` ミリ秒）で指定します                             |
...
既定の `RESULT_STORE=none` では、完了済みクエリの行データは `QUERY_TTL_MINUTES` のあいだだけサーバーメモリに残ります。
...
Parquet の object key と期限は JSONL の参照と別に管理され、rows、search、download は JSONL を読みます。
profile は条件を満たす場合だけ Parquet の行ストリームを使い、同じ profileRowsStream へ渡します。
...
JSONL の upload 完了後、`query_history` の JSONL link と `result_parquet_conversion_jobs` の pending 行を同じ DB transaction で保存します。
この transaction は Parquet 変換の完了を待たないため、変換中も既存の JSONL route が結果を返します。
worker は起動時と周期実行で pending 行を読み、source link、source key、期限、既存の Parquet link、列情報、形式を再検証してから変換します。
worker は単一 replica の直列実行を前提にしており、DB に running 状態を保存しません。
複数 replica で同時に worker を動かす場合は、lease または分散 claim を追加するまで有効にしないでください。
変換の失敗は retry 可能な `duckdb_error` などと、`unsupported_type`、`invalid_value`、`malformed_row`、`row_count_mismatch` などの恒久的な失敗に分類します。
retry の attempts が上限に達した job は `dead` として残り、履歴の JSONL link は残ります。
source の失効、source link の変更、履歴行の削除は obsolete 扱いで job row を削除し、列情報または形式の欠落は `dead` として残します。
Parquet link の確定後も job row を削除するため、成功したクエリ数や obsolete 件数に比例して job table が増え続けることはありません。
shutdown による `aborted` は attempts を消費せず、pending のまま次回起動へ残します。
converter timeout の `timed_out` は通常の retry として attempts を消費し、上限到達時は `dead` になります。
worker は変換用の一時ディレクトリを所有し、成功、失敗、abort のいずれでも削除します。
Parquet upload 後に link できなかった object は live reference を再確認して削除し、削除に失敗した場合は `result_object_deletions` へ移します。
Parquet が未作成、変換中、または job が dead でも JSONL が唯一の読み出し経路として残るため、結果 route は JSONL fallback を継続します。
アップロードに失敗してもクエリ自体は成功のまま扱い、warn ログと `query.result.persist` の監査ログに失敗を記録します。
保存に成功した履歴は History パネルから再実行なしで開けます。
CSV ダウンロードも保存済み結果が残っている場合は再実行より保存済み結果を優先します。
保存済み結果の復元時も owner チェックと現在 role の datasource allowlist を再評価します。
ワークフローのステップ結果も同じ S3 ストアと `RESULT_STORE_TTL_DAYS` の TTL で保存され、期限切れ掃除も共通です。
ワークフロー run の一括エクスポート (CSV zip / XLSX / Google Sheets) も、期限内の永続化済みステップ結果を S3 から読み出して出力します。

### 9.4.1 永続 profile の DuckDB 行ストリーミング

`GET /api/queries/:id/profile` は、認可、datasource allowlist、結果期限、304 判定を終えた後に、条件を満たす Parquet artifact を DuckDB から読み出せます。

この経路は集計 SQL を実行しません。
DuckDB は `read_parquet(?)` で生成済みの物理列 `c0000` から `cNNNN` を行単位のチャンクで読み、既存の `profileRowsStream` へ渡します。
そのため、文字列化、UTF-16 の先頭 100 code units、distinct 追跡上限 10,000、overflow 後の既知値の計数、top values の順位、min/max の比較は JSONL 経路と同じです。

Parquet 経路を選ぶ条件は、`RESULT_STORE=s3`、有効期限内の `parquetRef`、encoding version 1、履歴列情報、生成規則と一致する object key、末尾が `/` の prefix です。
Parquet の KV metadata に保存した encoding version、row count、physical schema と履歴列情報が一致しない場合も JSONL へ戻ります。
行数は JavaScript の safe integer として検証します。

Parquet が無い、期限切れ、encoding version 未対応、設定不一致などの適用外理由と、認証、httpfs、S3、timeout、schema mismatch、DuckDB の実行失敗は JSONL fallback の理由として分類します。
リクエスト abort は DuckDB connection を interrupt して処理を終了し、JSONL へ戻しません。
DuckDB と JSONL の両方が失敗した場合、内部ログには二つの失敗分類を残しますが、credential、secret、object URI は出力しません。

reader はリクエストごとに DuckDB instance、temporary secret、一意な temp directory を作り、threads 1、memory limit、temp size limit、timeout、同時実行数の上限を適用します。
temporary secret は設定された S3 prefix に限定し、credential chain は `CHAIN 'env'` に固定します。
`RESULT_PROFILE_DUCKDB_ENABLED` の既定値は `false` であり、本番で有効化する場合は static environment credential または同じ staging identity による probe を先に確認してください。

profile の ETag は従来の JSONL profile と共通で、Parquet の参照を含めません。
同じ profile 意味論のまま artifact の読み出し先だけが変わっても、ETag は変わりません。

### profile の観測と capability cache

profile 経路は request ごとに typed observer event を 1 件通知します。
event のフィールドは `route`、`outcome`、`reason`、`failureCode`、`totalDurationMs`、`queueWaitMs`、`duckdbDurationMs`、`jsonlFallbackDurationMs`、`rowCountBucket`、`cacheHit` です。
`route` は常に `profile`、`outcome` は `success`、`not_applicable`、`fallback`、`aborted` のいずれかです。
`rowCountBucket` は `0`、`1-999`、`1000-9999`、`10000-99999`、`100000+` のいずれかで、時間は負数にならないミリ秒です。
`queueWaitMs` は reader の semaphore 取得まで、`duckdbDurationMs` は取得後の DuckDB reader と cleanup、`jsonlFallbackDurationMs` は route が JSONL profile を完了するまでを表します。
observer の event に history id、owner、bucket、object key、S3 URI、credential、secret、SQL text は含めません。
observer の失敗は request の応答を変えません。

`auth` と `httpfs` は artifact に依存しない capability failure として process 内の bounded negative cache に保存します。
global cache の key は credential provider の種別、endpoint、region、bucket、prefix から作り、object key と credential 値を含めません。
`schema_mismatch` は object 単位の cache へ保存しますが、object key は SHA-256 の短い hash に変換するため、key や log label に raw object key を出しません。
global cache の既定 TTL は 60 秒、object の `schema_mismatch` cache の既定 TTL は 10 秒です。
`RESULT_PROFILE_DUCKDB_NEGATIVE_CACHE_TTL_MS=0` で両方を無効にできます。
cache へ入れるのは `auth`、`httpfs`、object 単位の `schema_mismatch` だけです。
`aborted`、`overloaded`、`timeout`、S3 object failure は cache しません。
cache hit は元の failure code を `reason` に残し、`cacheHit=true` として観測してから JSONL fallback を選びます。

### E-hardening の staging と canary 手順

E-hardening は profile route の HTTP 契約を変更しません。
production で Parquet profile を有効にする前に、同じ container image と同じ static environment credential provider を staging で使います。

1. `DuckDB direct S3 / MinIO credential gate` を実行し、raw read と product reader profile の両方が成功することを確認します。
2. staging の `RESULT_STORE`、bucket、prefix、region、endpoint、credential identity を production と同じ契約にそろえます。
3. `RESULT_PROFILE_DUCKDB_ENABLED=true` を 1 replica の canary process だけへ設定し、他の process は `false` のまま起動します。
4. canary で認可済みの保存結果を開き、JSONL profile と Parquet profile の row count、列順、null count、distinct、top values、min、max を fixture と実データで比較します。
5. typed observer の success、fallback reason、`cacheHit`、queue wait、DuckDB duration、JSONL fallback duration、timeout、abort を記録します。
6. `auth` または `httpfs` の fallback が発生した場合は flag を `false` に戻して process を再起動し、cache の有効期限を待つだけで再試行を続けないようにします。
7. canary の結果と benchmark の p50、p95 を記録し、S3 network latency と fresh DuckDB instance の起動時間を含まない benchmark を production latency の根拠にしません。
8. canary の観測結果が安定してから replica 単位で段階的に有効化し、abort が JSONL fallback を起動していないことを確認します。

rollback は `RESULT_PROFILE_DUCKDB_ENABLED=false` へ戻して process を再起動します。
flag の既定値は `false` なので、未設定の新規 process は JSONL 経路を使います。

### E1b の feasibility test

E1b の feasibility test は route を変更せず、local ZSTD Parquet を DuckDB から row streaming し、既存 `searchRowsStream` へ渡した結果を JSONL source と比較します。
比較 fixture には safe bigint、unsafe bigint、connector 由来の string bigint、REAL `0.1`、double、integer、boolean、varchar、NULL を含めます。
レスポンスは `offset`、`rows`、`totalMatched`、`totalRows`、`complete` を JSON 化して byte-equivalent かを判定します。

2026-07-14 の実測では byte-equivalent になりませんでした。
JSONL の safe bigint `42` は number ですが、Parquet row stream では string の `"42"` になりました。
REAL の `0.1` は Parquet row stream で `0.10000000149011612` になりました。
そのため test は raw-row canonicalization 未確定の間、E1b route を実装可能とは扱わず、実装を延期する証拠として失敗理由を固定します。
共通 canonical form または JSONL wire representation の復元契約を決めるまで、E1b は parity test と呼びません。

E1b の route 実装と本番 flag は、raw-row 契約と S3 end-to-end benchmark の設計レビューが完了するまで追加しません。

### profile の core benchmark

次の command は rollout 判断用の informative evidence を JSON で出力します。

```bash
pnpm --filter @hubble/server bench:duckdb-profile
```

出力 schema は `e-hardening-profile-benchmark-v1` です。
`cases` に small、medium、large、high-cardinality の dataset ごとに cold と warm、JSONL direct、controlled JSONL fallback、DuckDB Parquet の case を出します。
各 case には `rowCountBucket`、p50、p95、`queueWaitMs`、`duckdbDurationMs`、`jsonlFallbackDurationMs`、`peakRssBytes`、`tempBytes`、`fallbackReason` を含めます。
queue wait は同時実行圧力なしの benchmark では 0 になるため、値とともに制約へ記録します。
`peakRssBytes` は case 前後の RSS sample、`tempBytes` は benchmark temporary directory の観測値であり、native heap trace や spill 不在の証明ではありません。
controlled fallback の reason は `benchmark_controlled_auth` で、実運用 route の S3 障害率を表しません。
benchmark は S3 network、metadata validation を含みません。
cold は fresh DuckDB instance の生成を含み、warm も sample ごとに instance を生成するため、production の end-to-end latency を測るものではありません。
CI の hard threshold にはせず、canary の observer event と同じ dataset、列、warmup、repeat 数で p50 と p95 を比較します。

最小設定は次の通りです。

```bash
RESULT_STORE=s3
RESULT_STORE_S3_BUCKET=hubble-prod-results
RESULT_STORE_S3_PREFIX=hubble-results/
RESULT_STORE_TTL_DAYS=7
```

`RESULT_STORE_S3_REGION` と `RESULT_STORE_S3_ENDPOINT` は任意です。
`RESULT_STORE_S3_ENDPOINT` を設定した場合は S3 互換ストレージ向けに path-style request を使います。
認証は AWS SDK の標準 credential provider chain を使います。
ECS task role、EKS IRSA、EC2 instance profile、`AWS_ACCESS_KEY_ID` など、実行環境の標準的な方法で権限を渡してください。

必要な S3 権限は prefix を絞って付与できます。
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:s3:::hubble-prod-results/hubble-results/*"
    }
  ]
}
```

Hubble は起動時と 1 日 1 回、`result_expires_at` または `parquet_expires_at` を過ぎた object をそれぞれ削除します。
JSONL の削除成功時は `result_object_key`、`result_expires_at`、列情報、形式を NULL に戻します。
Parquet の削除成功時は `parquet_object_key` と `parquet_expires_at` だけを NULL に戻します。
片側の削除に失敗した場合は成功した側だけを NULL にし、失敗した側を次回の期限掃除へ残します。
履歴行は両方の artifact 参照が NULL になるまで retention の prune 対象になりません。
Workflow の明示削除と retention は、step row を消す transaction 内で object key を `result_object_deletions` へ登録します。
削除 worker は outbox を1分ごとに確認し、失敗時は backoff 付きで再試行します。

Parquet 変換 job の状態は次で確認できます。

```sql
SELECT history_id, source_object_key, target_object_key, status, attempts,
       next_attempt_at, last_error_code, last_error, updated_at
FROM result_parquet_conversion_jobs
ORDER BY next_attempt_at, history_id;
```

原因を修正した dead job だけを運用者が replay できます。
replay 前に source の JSONL link、列情報、形式、期限、object の存在を確認してください。
...
WHERE history_id = '<history-id>' AND status='dead';
```

obsolete 扱いの job は row 自体が削除されるため、履歴の JSONL link を復旧しても自動 replay されません。
再変換が必要な場合は、対象履歴に対する新しい pending job を運用手順または管理 API から登録してください。
dead row は保持期間を過ぎると worker が bounded に削除します。

pending outbox が残っている間は、元の result store へ再接続できる設定と `s3:DeleteObject` 権限を保持してください。
`RESULT_STORE=none` へ切り替えた場合、Hubble は object を削除済みとして扱わず、outbox を残して警告します。
PostgreSQL では次の SQL で保留件数と直近の失敗を確認できます。

```sql
SELECT object_key, attempts, next_attempt_at, last_error
FROM result_object_deletions
ORDER BY next_attempt_at, object_key;
```

S3 側にも lifecycle rule を併用すると、アプリ停止中に期限を過ぎた object の保険になります。
DB の key を NULL に戻す処理は Hubble 側が行うため、lifecycle rule は同じ日数か少し長めに設定してください。
...
旧key IDを持つ暗号文と旧3要素形式は、接続利用時にactive keyへ遅延再暗号化されます。
旧鍵をkeyringから削除する前に、長期間利用されていないGitHub接続を含めて再暗号化または再接続を完了してください。

`query_history` の `result_object_key`/`result_expires_at` は正規 JSONL、`parquet_object_key`/`parquet_expires_at` は派生 Parquet の参照だけを保持します。
`parquet_encoding_version` は派生 Parquet の encoding contract を保持し、`result_parquet_conversion_jobs` は pending/dead の JSONL source、生成先 target、retry 状態を保持します。
現在の migration は SQLite と PostgreSQL の両方へ nullable な Parquet 列と部分索引を追加します。
既存行の Parquet 列は NULL のまま移行され、object store のコピーや Parquet 変換は行いません。
変換後の Parquet object は元の JSONL key と `finished` 状態を確認してからリンクし、期限は JSONL の期限を引き継ぎます。
...
sqlite3 -noheader /var/backups/hubble-migration.db "
  ...
  UNION ALL
  SELECT parquet_object_key FROM query_history WHERE parquet_object_key IS NOT NULL
  UNION ALL
