...
プロセスは 1 つで完結します。永続化は必須の PostgreSQL（`DATABASE_URL`）で、データソースは `datasources.yaml`
...
| 永続化 DB            | PostgreSQL（`DATABASE_URL` 必須）                                                                                             |
...
`rbac.yaml`（`RBAC_PATH` が指すファイル）も必須です。各 role の permissions と
`datasources` allowlist を明示してください（例は `rbac.yaml.example`、Compose の最小例は
`deploy/compose/rbac.yaml`）。
...
Environment=RBAC_PATH=/etc/hubble/rbac.yaml
...
| `DATABASE_URL`                    | —                                 | PostgreSQL接続文字列（`postgres://` または `postgresql://`、必須）。それ以外のスキームは起動時エラー（[§9.1](#91-postgresql)）                                                                                                |
...
| `RBAC_PATH`                       | （未設定 = `./rbac.yaml`）        | RBAC 定義 YAML のパス。未設定時のパスにファイルが無ければ起動時エラー                                                                                                                                                         |
...
各ロールの `datasources` は必須です。`['*']` は全 datasource、`[]` は全 datasource の拒否、
id の列挙は指定した datasource のみを許可します。拒否時は datasource の存在有無を
...
```bash
AUTH_MODE=proxy DATABASE_URL=postgres://hubble:hubble@localhost:5432/hubble PORT=8082 \
  pnpm --filter @hubble/server start &
...
ではない）は PostgreSQL（必須の `DATABASE_URL`）のみです。`postgres://` または
`postgresql://` 形式以外のスキームは起動時エラーで停止します。起動時には PostgreSQL
バックエンドであることをログに1行出力します。

保存内容は Notebook、保存クエリ、Dashboard、Workflow、Schedule、Alert、共有設定、GitHub 接続、履歴、監査証跡です。
...
スキーマは `packages/server/migrations` を起動時に自動適用します（`schema_migrations` テーブルで適用管理）。
...
監査ログはインシデント調査と運用確認に使うデータなので、PostgreSQL の `pg_dump` の対象から除外しないでください。

### 9.3 クエリ結果保存 S3 バックエンド

#### 現行 baseline の適用確認

新規データベースでは `packages/server/migrations/0001_baseline.sql` が現行スキーマを作成します。
過去の migration 番号を順に適用する運用や、DuckDB / Parquet artifact 用の migration はありません。
既存の再作成可能なデータベースは、同じ baseline を空の PostgreSQL へ適用して作成します。
...
PostgreSQL では `information_schema.columns` で `result_object_key`、`result_expires_at`、
`result_columns_json` があり、`result_format`、Parquet 参照列、`result_parquet_conversion_jobs` がないことを確認します。
...
| **DB 接続 / lock 待ち**                     | `DATABASE_URL` の接続先と PostgreSQL の接続数、advisory lock 待ち、migration error を確認してください。                                                                                                                                                                                |
| **静的配信されない**                        | 起動ログに `serving static web app from …` が出ているか、`STATIC_DIR` が `index.html` を含むディレクトリ（= `web build` 済み）を指しているかを確認                                                                                                                                     |
| **メタデータが古い / 詰まる**               | Data パネルの更新ボタンか対象 datasource の `POST /api/datasources/:datasourceId/metadata/refresh`。詰まりは [§8](#8-trino-resource-group-分離メタデータ取得詰まり対策) の resource group 分離で緩和                                                                                   |
...
旧形式の token は Hubble で復号も再暗号化もできません。旧形式の接続は GitHub OAuth で再接続し、
新しい v1 形式の token を発行してから旧データを整理します。v1 形式で旧 key ID を持つ暗号文だけは、
keyring に旧鍵を残すことで利用時に active key へ遅延再暗号化できます。
...
provider を有効化して利用するには、`rbac.yaml` の role に `ai.use` を付与します。
リクエスト対象の datasource に対する allowlist チェックも通常クエリと同様に適用されます。
