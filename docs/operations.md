...
| `RESULT_STORE`                    | `none`                            | クエリ結果の保存先（`none` / `s3`）。`s3` のとき完了結果を gzip JSONL で S3 へ保存し、履歴から再実行なしで開けます                                                                                                              |
...
`audit_log` は操作回数に比例して増えるため、保持期間を決めて定期削除してください。

```sql
-- PostgreSQL: 180 日より古い監査ログを削除
DELETE FROM audit_log
WHERE created_at::timestamptz < now() - interval '180 days';

-- SQLite: 180 日より古い監査ログを削除
DELETE FROM audit_log
...
`RESULT_STORE=s3` を有効にすると、クエリが正常終了したあとに全結果を gzip JSONL として S3 へストリーミング保存します。
...
Hubble は起動時と 1 日 1 回、`result_expires_at` を過ぎた object を削除し、DB の `result_object_key` と `result_expires_at` を NULL に戻します。
...
3. `GITHUB_TOKEN_ENCRYPTION_KEY` を移行前後で同じ値に保管します。
...
暗号鍵を変更すると、移行した GitHub 接続を復号できません。

`query_history` と `workflow_step_runs` の `result_object_key` は、S3 object 自体ではなく参照だけを保持します。
...
1. migration error がなく `/api/healthz` が成功すること。
