...
定時同期も server プロセスごとに cron タイマーを持ちます。複数レプリカで同じ cron が
重複実行されないよう、スケジューラーと同様 **定時取り込みは 1 プロセスのみで有効** にするか、
`GITHUB_SYNC_CRON=off` で無効化してください ([§12](#12-クエリスケジューラー) の
`SCHEDULER_ENABLED` と同様の運用)。
