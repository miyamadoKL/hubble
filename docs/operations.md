...
| `GITHUB_GOVERNANCE`               | `off`                             | ガバナンス強制の予約フラグ (`off` / `on`)。フェーズ 1 では UI 表示のみで、未承認ドキュメントの実行制限などは未実装                                                                                                              |
...
保存内容はどちらのバックエンドでも notebook、保存クエリ、履歴、監査証跡です。
...
### 9.6 SQLite から PostgreSQL への移行（概略）

SQLite → PostgreSQL の自動移行ツールは提供していません。保存内容は
`notebooks` / `saved_queries` / `query_history` / `audit_log` の 4 テーブルのみのため、手動移行は
次の手順が目安です。

1. Hubble を停止し、SQLite ファイルのオンラインバックアップを取る（§9.2）。
2. `sqlite3 hubble.db .dump` 等で対象 4 テーブルのデータをエクスポートする。
...
- `GITHUB_GOVERNANCE=on` によるガバナンス強制 (未承認ドキュメントの結果永続化制限、cron 実行の
  approved 必須化など)
- デフォルトブランチから Hubble への逆方向取り込み (pull / sync)
