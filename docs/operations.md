...
```bash
PORT=8080 \
DB_PATH=/var/lib/hubble/hubble.db \
...
Environment=DB_PATH=/var/lib/hubble/hubble.db
...
| `SCHEDULER_ENABLED`               | `true`                     | `false` にするとスケジューラーの tick ループを起動しない（API は生きたまま、スケジュールの登録・閲覧は可能）                                                                                           |
...
| `TRINO_SCHEDULED_SOURCE`          | `hubble-scheduled`         | スケジュール実行の `X-Trino-Source`（resource group 分離に使用）                                                                                                                                       |
...
（既定値。`TRINO_SOURCE` / `TRINO_METADATA_SOURCE` / `TRINO_SCHEDULED_SOURCE` で変更可。）そのため、queryText 依存の
...
`hubble-scheduled`（`TRINO_SCHEDULED_SOURCE` で変更可）を付与します。ロール解決は owner
