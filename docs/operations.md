...
| `csv.download`         | 結果 CSV のダウンロード要求と、必要に応じた再実行の判断。   |
...
現行実装では、CSV ダウンロード再実行不能の拒否は `csv.download` の `outcome: "denied"` として残り、スケジュールの失敗または Query Guard による block は `schedule.execute` の `outcome` と `errorType` で追跡できます。
...
### 9.5 SQLite から PostgreSQL への移行（概略）
...
### 9.6 none → proxy 切替時の owner backfill
