...
#### fast path の配備前 zero gate

0023 を含む release は、既存環境の zero gate を通過した場合だけ配備します。
この release は既存の Parquet 派生 artifact を移行しないため、次の件数がすべて 0 であることを確認します。

```sql
SELECT
  (SELECT COUNT(*)
   FROM query_history
   WHERE parquet_object_key IS NOT NULL) AS legacy_parquet_refs,
  (SELECT COUNT(*)
   FROM result_parquet_conversion_jobs) AS conversion_jobs,
  (SELECT COUNT(*)
   FROM result_object_deletions AS d
   WHERE d.object_key LIKE '%.parquet'
      OR EXISTS (
        SELECT 1
        FROM query_history AS h
        WHERE h.parquet_object_key = d.object_key
      )) AS parquet_deletion_jobs;
```

既存の結果 store が S3 の場合は、結果 prefix の object listing または S3 Inventory で `.parquet` object 件数を確認します。

```bash
set -euo pipefail
: "${RESULT_STORE_S3_BUCKET:?RESULT_STORE_S3_BUCKET must be set}"
result_prefix="${RESULT_STORE_S3_PREFIX:-hubble-results/}"
case "$result_prefix" in
  */) ;;
  *) echo "RESULT_STORE_S3_PREFIX must end with /" >&2; exit 1 ;;
esac
aws_s3_global_args=()
if [[ -n "${RESULT_STORE_S3_ENDPOINT:-}" ]]; then
  aws_s3_global_args+=(--endpoint-url "$RESULT_STORE_S3_ENDPOINT")
fi
parquet_objects="$(
  aws s3 ls "s3://${RESULT_STORE_S3_BUCKET}/${result_prefix}" --recursive "${aws_s3_global_args[@]}" |
    awk '$4 ~ /\.parquet$/ { count += 1 } END { print count + 0 }'
)"
test "$parquet_objects" -eq 0
```

object listing または S3 Inventory を取得できず件数を確定できない場合も gate failure とします。
いずれかの DB 件数または object 件数が 0 でない場合、この release を配備しません。
その環境では R0/R1 の停止と cleanup を別 release で完了し、zero gate を再確認してから本 release を適用します。
zero gate の SQL と object 件数を PR body に記載し、レビュアーが同じ判定を再現できる状態にします。
...
JSONL object の削除に成功すると、`query_history` の object key、期限、列情報、形式を NULL に戻します。
...
migration 0021 と 0022 は既存データを読み取れるよう変更せず、追加済みの非稼働列とテーブルを残します。
migration 0023 はデータを削除せず、非稼働経路専用の index を削除して JSONL 用 retention index を再作成します。
既存の非稼働列とテーブルは、将来の別リリースで削除方針が決まるまで tombstone として保持します。

運用で確認する結果参照は、次の SQL で取得できます。

```sql
SELECT id, result_object_key, result_expires_at, result_format
FROM query_history
...
現在の application table は次の17表です。
...
| 互換性用 tombstone      | `result_parquet_conversion_jobs`                                                 |
...
`query_history` の `result_object_key`/`result_expires_at` が現行の JSONL 結果参照です。
0021 と 0022 で追加された非稼働の列とテーブルは、既存 schema との互換性を保つため残しています。
0023 は既存行や object を削除せず、非稼働経路専用の index を削除して JSONL 用 retention index を再作成します。
