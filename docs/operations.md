...
既存の再作成可能なデータベースは、同じ baseline を空の SQLite または PostgreSQL へ適用して作成します。
...
SQLite では `PRAGMA table_info(query_history)` に `result_object_key`、`result_expires_at`、`result_columns_json` があり、
`result_format`、`parquet_object_key`、`parquet_expires_at`、`parquet_encoding_version` がないことを確認します。
PostgreSQL では `information_schema.columns` で同じ列構成を確認します。
`result_parquet_conversion_jobs` は両方の DB に存在しません。
...
現行 baseline では `result_object_key`、`result_expires_at`、`result_columns_json` を保持し、
`result_format`、Parquet 参照列、`parquet_encoding_version`、変換ジョブテーブルは作成しません。
