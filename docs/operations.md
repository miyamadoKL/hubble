...
保存内容はどちらのバックエンドでも**要約と監査証跡のみ**です: `notebooks` / `saved_queries` /
`query_history`（と各セルの最終実行サマリ）/ `audit_log`。**結果の行データは保存しません**
...
### 9.4 SQLite から PostgreSQL への移行（概略）
...
### 9.5 none → proxy 切替時の owner backfill
