...
| `deploy/compose/rbac.yaml`         | Compose 用の明示的な demo RBAC 設定（全 datasource allowlist）      |
...
過去の migration 番号を順に適用する運用はありません。
結果参照は `query_history.result_object_key`、`result_expires_at`、`result_columns_json` を使い、
Parquet の列や変換ジョブテーブルは作成されません。
...
docker run --rm \
  ...
  -v "$(pwd)/rbac.yaml:/etc/hubble/rbac.yaml:ro" \
  -e DATASOURCES_PATH=/etc/hubble/datasources.yaml \
  -e RBAC_PATH=/etc/hubble/rbac.yaml \
...
# データソース一覧を確認し、表示された id でカタログ一覧を取得
curl -s http://localhost:8080/api/datasources
curl -s http://localhost:8080/api/datasources/<datasource-id>/catalogs
```
...
```bash
curl -s -o /dev/null -w "%{http_code}\n" http://localhost:8080/api/healthz   # → 200
curl -s http://localhost:8080/api/datasources/trino-default/catalogs       # tpch を含む
```
