...
test('Schedule 作成モーダル: 英語(既定)→日本語切替でラベルが翻訳され、どちらでも overflow しない', async ({
  ...
  // schedule は必ず savedQueryId を参照するため、New schedule ボタンを有効化するには
  // 参照できる保存済みクエリが最低 1 件必要（SchedulesPanel は 0 件だとボタンを無効化する）。
  await seedSavedQuery(request, {
    name: `i18n modal source ${rnd()}`,
...
test('SchedulesPanel/AlertsPanel の行表示、状態バッジ、実行履歴モーダル、TopBar が日本語化で overflow を悪化させない', async ({
  ...
  const scheduleSavedQueryId = await seedSavedQuery(request, {
    name: `i18n layout schedule source ${rnd()}`,
    statement: 'SELECT count(*) FROM tpch.tiny.nation',
  });
  await seedSchedule(request, {
    name: scheduleName,
    savedQueryId: scheduleSavedQueryId,
  });
