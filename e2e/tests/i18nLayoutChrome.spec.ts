...
test('ShareModal: 保存済みクエリの共有先編集が日英で翻訳され、overflowしない', async ({
  ...
  // 起動元の「Share」ボタン自体は SavedQueriesPanel（components/panels/、本フェーズの
  // 対象外）が持つ固定英語文言なのでロケールを切り替えても翻訳されない。
  // ShareModal 本体（このフェーズの対象）だけが日本語化されることを確認する。
  await page.getByRole('button', { name: 'Share', exact: true }).click();
...
test('ShortcutsHelp: コマンドパレット経由で開いたヘルプが日英で翻訳され、overflowしない', async ({
  ...
  // CommandPalette 自体（components/palette/）は本フェーズの対象外なので、
  // コマンド名やプレースホルダーは英語のままで開く。開いた先の ShortcutsHelp
  // 本体だけが日本語化されていることを確認する。
  await page.keyboard.press('Control+k');
  await page.getByPlaceholder('Type a command…').fill('Keyboard shortcuts');
  await page
    .getByRole('dialog', { name: 'Command palette' })
