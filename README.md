...
- **結果の永続化（任意）** — `RESULT_STORE=s3` で完了済みクエリの全結果を zstd 圧縮 JSONL として S3 へ保存し、履歴から再実行なしで開けます。
...
- **結果は既定でメモリ上に保持**: `RESULT_STORE=none` では行データは server のメモリ + SSE に存在し、PostgreSQL
  にはサマリ（notebooks、saved queries、history、セルごとの `resultMeta`）のみを
...
手元の Node で server/web を個別に立ち上げる場合は、`datasources.yaml` と PostgreSQL を用意してから起動します。
...
| `DATABASE_URL`                    | —                    | `postgres://` / `postgresql://` 形式の接続文字列（必須）。Hubbleの永続化バックエンドはPostgreSQLのみ                       |
...
# E2Eには通常BFF用とproxy認証BFF用の別PostgreSQL DB URLが必要。
...
# 稼働中のTrino（tpch）に対するE2E。server（PostgreSQL）とwebを自動起動する。
pnpm --filter @hubble/e2e test

# マルチデータソースE2E（任意。上の既定コマンドでは実行しない）。
# 先にデモDBを起動する: docker compose -f docker-compose.yml -f docker-compose.demo.yml --profile demo up -d demo-mysql demo-postgres
...
します。通常 BFF 用に `E2E_DATABASE_URL` または `TEST_DATABASE_URL` で専用の PostgreSQL
DBを指定し、proxy 認証 BFF 用には必須の `E2E_AUTH_DATABASE_URL` で別の PostgreSQL DBを
指定します。2つのURLは一致させません。`QUERY_MAX_ROWS=10000` で truncation の経路を確認します。
